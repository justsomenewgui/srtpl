...
while (my $line = <FILE>) {
    if ($line =~ /-?\d*\d:\d\d:\d\d,\d\d\d\s-->\s-?\d*\d:\d\d:/) {
        my $first = timecodes(9, $line, $file, $seconds, $milliseconds,
                  $minutes);
        my $second = timecodes(26, $line, $file, $seconds, $milliseconds,
                   $minutes);
        $line = "$first --> $second\n";
    push(@lines, $line);
    }
...
sub timecodes ($x, $line, $file, $seconds=0, $milliseconds=0, $minutes=0) {
